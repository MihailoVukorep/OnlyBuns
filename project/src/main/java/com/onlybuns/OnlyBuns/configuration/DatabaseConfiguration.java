...
public class DatabaseConfiguration {
    ...
    public boolean instantiate() {
        ...
        // RANDOM POSTS
        repository_post.save(new Post("Bunny Care 101", "How often should I clean my rabbit's cage?", acc_pera));
        repository_post.save(new Post("Favorite Bunny Breeds", "What's your favorite breed? I'm partial to Holland Lops!", acc_pera));
        repository_post.save(new Post("Funny Bunny Moments", "My bunny just did the craziest binky! What's the funniest thing your bunny has done?", acc_pera));
        repository_post.save(new Post("Healthy Bunny Treats", "Are there any treats that are safe for rabbits but also super healthy?", acc_pera));
        repository_post.save(new Post("DIY Bunny Toys", "I made a cardboard castle for my bunny, and they love it! Any other DIY toy ideas?", acc_pera));
        repository_post.save(new Post("First Time Bunny Owner", "Just got my first rabbit! Any tips for bonding with my new furry friend?", acc_pera));
        repository_post.save(new Post("Bunny Housing Advice", "Is it better to have a pen or free roam in a bunny-proof room?", acc_pera));
        repository_post.save(new Post("Rabbits and Other Pets", "Can rabbits get along with cats? I'd love to hear your experiences.", acc_pera));
        repository_post.save(new Post("Bunny Behavior Help", "My bunny keeps thumping at night. What does it mean?", acc_pera));
        repository_post.save(new Post("Best Bunny Photos!", "Let's have a thread of the cutest bunny photos! Here's mine to start!", acc_pera));
        repository_post.save(new Post("Best Hay Brands?", "What's the best brand of hay for picky rabbits?", acc_pera));
        repository_post.save(new Post("Are Bunnies Nocturnal?", "My rabbit is super active at night. Is this normal?", acc_pera));
        repository_post.save(new Post("Favorite Bunny Names", "What are some of the cutest bunny names you've heard?", acc_pera));
        repository_post.save(new Post("Litter Training Tips", "How did you teach your bunny to use the litter box?", acc_pera));
        repository_post.save(new Post("Bunny Zoomies!", "My bunny has been running in circles nonstop. Is this a good sign?", acc_pera));
        repository_post.save(new Post("Vegetables for Bunnies", "Which veggies are safe to feed daily?", acc_pera));
        repository_post.save(new Post("Are Rabbits Affectionate?", "How do rabbits show they love you?", acc_pera));
        repository_post.save(new Post("Bunny Bonding Stories", "How long did it take for your rabbits to bond?", acc_pera));
        repository_post.save(new Post("Rabbit-Proofing Tips", "What's the best way to protect my wires and furniture?", acc_pera));
        repository_post.save(new Post("Should I Get Two Rabbits?", "Is it better to keep rabbits in pairs?", acc_pera));
        repository_post.save(new Post("How to Groom a Bunny?", "My rabbit hates being brushed. Any advice?", acc_pera));
        repository_post.save(new Post("Fun Bunny Activities", "What games do your rabbits enjoy?", acc_pera));
        repository_post.save(new Post("Bunny Birthday Ideas", "How do you celebrate your rabbit's birthday?", acc_pera));
        repository_post.save(new Post("Rabbits and the Heat", "What's the best way to keep rabbits cool in summer?", acc_pera));
        repository_post.save(new Post("Winter Bunny Care", "Do indoor rabbits need anything special in winter?", acc_pera));
        repository_post.save(new Post("Safe Bunny Snacks", "Are there any store-bought treats you recommend?", acc_pera));
        repository_post.save(new Post("Bunny Habitat Size", "How much space does a single rabbit need?", acc_pera));
        repository_post.save(new Post("Traveling with Rabbits", "Has anyone traveled long-distance with their bunny? Tips?", acc_pera));
        repository_post.save(new Post("Adopt or Shop?", "What are the benefits of adopting a bunny?", acc_pera));
        repository_post.save(new Post("Do Rabbits Like Music?", "I noticed my rabbit gets calmer when I play soft music. Does yours?", acc_pera));
        repository_post.save(new Post("Bunny-Safe Plants", "What houseplants are safe to have around rabbits?", acc_pera));
